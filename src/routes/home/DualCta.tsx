...
export function DualCta() {
  ...
  return (
    <Section className="pb-24 pt-20 sm:pb-28">
      <Container>
        <div className="relative grid gap-px border border-greige bg-greige md:grid-cols-2">
          {[dealers, story].map((block) => (
            <Link
              key={block.cta.to}
              to={block.cta.to}
              className="group flex flex-col justify-between bg-paper p-9 transition-colors hover:bg-ivory sm:p-12"
            >
              <div>
              ...
              <span className="mt-10 text-[0.625rem] uppercase tracking-(--tracking-eyebrow) text-brass">
                {block.cta.label} &rarr;
              </span>
            </Link>
          ))}
        </div>
