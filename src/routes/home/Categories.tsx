  ...
  return (
    <Section className="py-20 sm:py-28">
      <Container>
        ...
        <ul className="mt-12 grid gap-px border border-greige bg-greige sm:grid-cols-2 lg:grid-cols-3">
          {site.categories.map((c) => (
            <li key={c.slug} className="bg-paper">
              <Link
                to={`/collections/${c.slug}`}
                className="group flex h-full flex-col p-7 transition-colors hover:bg-ivory"
              >
                ...
                <span className="mt-6 text-[0.625rem] uppercase tracking-(--tracking-eyebrow) text-brass">
                  Explore &rarr;
                </span>
              </Link>
            </li>
